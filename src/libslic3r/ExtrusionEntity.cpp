...
Polyline ExtrusionMultiPath::as_polyline() const
{
    ...
    if (! paths.empty()) {
        ...
        for (size_t i_path = 0; i_path < paths.size(); ++ i_path) {
            ...
            std::transform(
                paths[i_path].polyline.points.begin() + 1, paths[i_path].polyline.points.end(),
...
Polygon ExtrusionLoop::polygon() const
{
    ...
    for (const ExtrusionPath &path : this->paths) {
        ...
        std::transform(
            path.polyline.points.begin(), path.polyline.points.end()-1,
...
void ExtrusionLoop::split_at(const Point &point, bool prefer_non_overhang, const double scaled_epsilon)
{
    ...
    // Snap p to start or end of segment_idx if closer than scaled_epsilon.
    {
        ...
        const Point3 *p2 = p1;
        ++p2;
        double       d2_1 = (point - p1->to_point()).cast<double>().squaredNorm();
        double       d2_2 = (point - p2->to_point()).cast<double>().squaredNorm();
        const double thr2 = scaled_epsilon * scaled_epsilon;
        if (d2_1 < d2_2) {
            if (d2_1 < thr2) p = p1->to_point();
        } else {
            if (d2_2 < thr2) p = p2->to_point();
        }
    }
...
//     throw RuntimeError("ExtrusionPathContoured::clone() called");
// }
// ExtrusionEntity *ExtrusionPathContoured::clone_move() { 
//     throw RuntimeError("ExtrusionPathContoured::clone_move() called");
// }
// void ExtrusionPathContoured::simplify(double tolerance) { 
//     return;
//     // throw RuntimeError("ExtrusionPathContoured::simplify() called"); 
// };
// void ExtrusionPathContoured::simplify_by_fitting_arc(double tolerance) {
//     return;
//     //throw RuntimeError("ExtrusionPathContoured::simplify_by_fitting_arc() called");
// };
// void ExtrusionPathContoured::reverse() {
//     this->polyline.reverse();
//     std::reverse(this->z_diffs.begin(), this->z_diffs.end());
//     // throw RuntimeError("ExtrusionPathContoured::reverse() called");
// }
...
void ExtrusionMultiPath::collect_points(Points &dst) const {
    size_t n = std::accumulate(paths.begin(), paths.end(), 0, [](const size_t n, const ExtrusionPath &p) {
        return n + p.polyline.size();
    });
    dst.reserve(dst.size() + n);
    for (const ExtrusionPath &p : this->paths) {
        for (const Point3 &point : p.polyline.points) {
            dst.emplace_back(point.x(), point.y());
        }
    }
