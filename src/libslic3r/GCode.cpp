    ...
    if (m_wipe.enable && FILAMENT_CONFIG(wipe)) {
        ...
        if (is_tree(this->config().support_type) && (path.role() == erSupportMaterial || path.role() == erSupportMaterialInterface || path.role() == erSupportTransition)) {
            if ((m_wipe.path.first_point() - m_wipe.path.last_point()).cast<double>().norm() > scale_(0.2)) {
                ...
                int    i        = 0;
                for (; i < (int)path.polyline.points.size(); i++) {
                ...
                Polyline sub;
                sub.append(path.polyline.points.begin() + std::max(0, i - 1), path.polyline.points.end());
                m_wipe.path = std::move(sub);
            }
        } else
            m_wipe.path.reverse();
    }
                ...
                for (const Line3& line : path.polyline.lines()) {
                    ...
                    if (path.z_contoured) {
                        ...
                        gcode += m_writer.extrude_to_xyz(
                            ...
                            e,
                            GCodeWriter::full_gcode_comment ? (tempDescription + "; z_diff " + std::to_string(z_diff) + " " + ExtrusionEntity::role_to_string(path.role()) + "; eratio " + std::to_string(extrusion_ratio)) : "");
