    ...
    bool &steep_overhang_contour, bool &steep_overhang_hole, bool reverse_thin_wall_hole)
{
    ...
    for (const PerimeterGeneratorLoop &loop : loops) {
        ...
        if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers) {
            ...
            Point first_point = paths.front().first_point();
        ...
        if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers) {
            ...
            if (!paths.empty()) {
                ...
                if (!extrusion->is_closed) {
                    ...
                    for (const ExtrusionPath& path : paths) {
                        ++point_occurrence[path.polyline.first_point().to_point()].occurrence;
                        ++point_occurrence[path.polyline.last_point().to_point()].occurrence;
                        if (path.role() == erOverhangPerimeter) {
                            point_occurrence[path.polyline.first_point().to_point()].is_overhang = true;
                            point_occurrence[path.polyline.last_point().to_point()].is_overhang = true;
                        }
                    }
...
bool paths_touch(const ExtrusionPath &path_one, const ExtrusionPath &path_two, double limit_distance)
{
    ...
    for (size_t pt_idx = 0; pt_idx < path_one.polyline.size(); pt_idx++) {
        if (lines_two.distance_from_lines<false>(path_one.polyline.points[pt_idx].to_point()) < limit_distance) { return true; }
    }
    ...
    for (size_t pt_idx = 0; pt_idx < path_two.polyline.size(); pt_idx++) {
        if (lines_one.distance_from_lines<false>(path_two.polyline.points[pt_idx].to_point()) < limit_distance) { return true; }
    }
    return false;
}
            ...
            if (!overhang_region.empty()) {
                ...
                if (!first_overhang_is_closed_and_anchored) {
                    ...
                    for (size_t i = 0; i < overhang_region.front().polyline.size(); i++) {
                        Point p = overhang_region.front().polyline[i].to_point();
