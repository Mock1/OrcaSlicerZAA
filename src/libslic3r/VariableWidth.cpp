...
ExtrusionMultiPath thick_polyline_to_multi_path(const ThickPolyline& thick_polyline, ExtrusionRole role, const Flow& flow, const float tolerance, const float merge_tolerance)
{
    ...
    for (int i = 0; i < (int)lines.size(); ++i) {
        ...
        if (line_len < SCALED_EPSILON) {
            ...
            if (!path.empty())
                path.polyline.points.back() = Point3(line.b, 0); // If the variable path is non-empty, connect this tiny line to it.
                ...
                multi_path.paths.back().polyline.points.back() = Point3(line.b, 0); // Connect this tiny line to the last finished path.
